...
is("anything but the seven is refused", (await msg("POST", "", nick, { react: first.id, emoji: "🍕" })).status, 400);
const wow = await msg("POST", "", nick, { react: first.id, emoji: "😮" });
is("surprised is one of them", [wow.status, wow.data.reacted], [200, "😮"]);
is("...stored after the first six, so no older reaction changes meaning", (await db.prepare("SELECT loved FROM loves WHERE kind = 'm' AND target = ?1 AND person = 'nicholas'").bind(String(first.id)).first()).loved, 7);
